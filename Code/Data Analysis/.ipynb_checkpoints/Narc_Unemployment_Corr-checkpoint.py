...
#read in crime data
crime_df= pd.read_csv('nar_crime_updates.csv')
...
#divide narc crime per month by population / 1000 to crime rate per 1000 people
...
#print correlation of narcotic crime and unemployment
print(corr_df.corr())

#plot narc crime vs unemployment rate
x = corr_df['crime']
y = corr_df['unemployment']

plt.scatter(x, y)
plt.xlabel("Monthly Narcotic Crime Rate per 1000 people")
plt.ylabel("Monthly Unemployment Rate")
plt.title('Monthly Narcotic Crime Rate vs Monthly Unemployment Rate')

m, b = np.polyfit(x, y, 1)
plt.plot(x, m*x+b, color='red')

plt.savefig('Monthly Narcotic Crime Rate vs Monthly Unemployment Rate')
plt.clf()