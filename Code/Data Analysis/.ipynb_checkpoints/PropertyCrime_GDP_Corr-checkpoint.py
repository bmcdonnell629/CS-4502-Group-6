...
#read in crime data
crime_df= pd.read_csv('property_crime_updates.csv')
#read in gdp data
gdp_df = pd.read_csv('GDP_2006_2019.csv')

#read gdp rates into list
GDP_dict = gdp_df.to_dict('records')
gdp = []
for row in GDP_dict:
    gdp.append(row['GDP(Billion)'])

#get count of property crime grouped by year
crime = crime_df.groupby(['Year'])['Primary Type'].count().tolist()
#divide property crime per month by population / 1000 to crime rate per 1000 people
...
#print corr of property crime vs GDP
print(corr_df.corr())


#plot property crime vs gdp
x = corr_df['crime']
y = corr_df['GDP']

plt.scatter(x, y)
plt.xlabel("Annual Property Crime Rate per 1000 people")
plt.ylabel("GDP in Billions Adjusted to 2012 dollars")
plt.title('Annual Property Crime Rate vs GDP')

m, b = np.polyfit(x, y, 1)
plt.plot(x, m*x+b, color='red')

plt.savefig('Annual Property Crime Rate vs GDP')
plt.clf()
